    ...
    with gzip.open(report_path, "rt") as mpistat:
        ...
        for line in mpistat:
            ...
            if lines_read % 5000000 == 0:
                print(f"Read {lines_read} lines from {report_path}")

            # Decode the Path, Split it and See If We Care
            mpi_line_info = line.split()

            try:
                filepath = base64.b64decode(mpi_line_info[0]).decode("UTF-8", "replace")
            except:
                continue
            ...
            if ".vault" in path_elems and "-" in path_elems[-1]:
                ...
                try:
                    rel_path = base64.b64decode(path_elems[-1].split("-")[1])
                except:
                    # TODO
                    continue
                ...
                master_of_puppets[full_path] = VaultPuppet(path_elems[vault_loc + 1])
