...
def main(mode: str) -> None:
    ...
    while (days_ago < MAX_DAYS_AGO and success == 0):
        ...
        if os.path.isfile(f"{REPORT_DIR}report-output-files/report-{mpi_date}.tsv"):
            success = 2
            print(f"Can't find mpistat output more recent than report-{mpi_date}.tsv!")
        else:
            if not all_exists(mpi_date):
                ...
                if mode == "inspector":
                    ...
                    for volume in VOLUMES:
                        ...
                        try:
                            os.symlink(f"{MPISTAT_DIR}{mpi_date}_{volume}.dat.gz", new_link)
                        except OSError as e:
    ...
    if days_ago >= MAX_DAYS_AGO:
        print(f"No usable mpistat output sets from the last {days_ago} days found!")
